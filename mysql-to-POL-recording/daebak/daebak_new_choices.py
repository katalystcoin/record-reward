...
id_file = open('daebak_new_choice_ID.txt', 'r+')
...
DB_DATABASE = os.getenv('DBK_DATABASE')
...
try:
    with pol_cur , mysql_cur:
        sql = """
                SELECT uc.id, uc.WalletToReward, uc.UserFBID, usr.Name,
                j.Name as choice1, h.name as choice2, g.Name as userChoice
                FROM UserChoice uc
                	LEFT JOIN User usr ON uc.UserFBID = usr.UserFBID
                    INNER JOIN GameChoice g ON g.ChoiceCode = uc.UserChoice
                    INNER JOIN GameChoice j on j.ChoiceCode = uc.GameChoice1
                    INNER JOIN GameChoice h on h.ChoiceCOde = uc.GameChoice2
                WHERE uc.Id > %s;
                """
        ...
        if(length_all_result >= 1):
            ...
            for row in all_new_result:
                user = str('Anonymous') if row[3] is None else str(row[3])

                insert_into_pol_query = "INSERT INTO pol_record (app_name, data) VALUES('daebak', %s)"
                val =  'likes, {0}, {1} likes {2} when given a choice of {3} and {4}'\
                        .format( str(row[1]), user, str(row[4]), str(row[5]), str(row[6]) )
